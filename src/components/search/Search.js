...
export function Search() {
      ...
      return data
        ?.filter((product) => {
          if (product.title.toLowerCase().includes(search.toLowerCase())) {
            ...
            return true; // Return true to include the product in the filtered results
          }
          return false; // Return false if the product doesn't match the search
        })
        .map((product, index) => {
          return (
            <Link
  ...
  return (
    <>
      {searchField && (
        <div className="absolute left-0 right-0 top-0 bottom-0 w-full h-[100vh] z-10 bg-gray-300/50" onClick={toggleForm} />
      )}
